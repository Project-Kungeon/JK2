...
#include "Character/PC/Warrior/JK2Warrior.h"
...
void UJK2GameInstance::HandleSpawn(const message::ObjectInfo& info, bool isMyPlayer)
{
	...
	if (isMyPlayer)
	{
		...
		auto* warrior = World->SpawnActor(AJK2Warrior::StaticClass(), &SpawnLocation);
		auto* Player = Cast<AJK2Warrior>(warrior);
		APawn* warrior_pawn = Cast<APawn>(warrior);
		PC->Possess(warrior_pawn);

		//auto* Player = Cast<AJK2PlayerCharacter>(PC->GetPawn());
		...
		Cast<AJK2PlayerCharacter>(Player)->isConnected = true;
	...
	else
	{
		...
		auto* Player = Cast<AJK2PlayerCharacter>(a);
		Player->SetPlayerInfo(info.pos_info());
		Players.Add(info.object_id(), Player);
	}
}
...
void UJK2GameInstance::HandleMove(const message::S_Move& movePkt)
{
	...
	AJK2PlayerCharacter** FindActor = Players.Find(objectId);
	if ( FindActor == nullptr )
		return;

	AJK2PlayerCharacter* Player = (*FindActor);
